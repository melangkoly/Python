...
def pickle_dump():
    with open("./sample/players.bin", "wb") as f: # 자원 자동 정리, pickle 사용을 위해 모드를 B로
        data = {"baseball", 9}
        pickle.dump(data, f)
    print("Dumpe Success")
...
def pickle_dump_multi():
    #dump 메소드를 중복 실행하면 여러 객체를 dump 할 수 있다
    with open("./sample/players.bin", "wb") as f:
        pickle.dump({"baseball" : 9}, f, 1) # 프로토콜의 버전 명시 가능 지금은 버전 1
        pickle.dump({"basketball" : 5}, f, pickle.HIGHEST_PROTOCOL) # 가장 최신 버전의 프로토콜 사용
        pickle.dump({"soccer" : 11}, f) # 버전을 명시하지 않으면 스스로 가장 최신 버전을 사용

    print("중복 덤프 완료!")

def pickle_load_multi():
    # 파일 내부에 몇 개의 피클 객체가 저장되어 있는지 확인이 어렵다
    with open("./sample/players.bin", "rb") as f:
        # print(pickle.load(f))
        # print(pickle.load(f))
        # print(pickle.load(f))
